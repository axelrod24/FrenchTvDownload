...
class Tf1VideoMetadata(VideoMetadata):
    ...
    def parseMetadata(self):
        ...
        self._progTitle = self.get('content', {}).get("title",
                                                      "default_prog_title")
        self._progName = self.get('media', {}).get('owner',
                                                   "default_prog_name")
        self._duration = self.get('media', {}).get('duration', 0)

        data = self.get("mediametrie", {}).get("chapters", [{}])[0]
        gregorian_date = data.get('estatS4')
        self._airDate = time.mktime(
            datetime.strptime(gregorian_date, "%Y-%m-%d").timetuple())
        self._synopsis = "%s\r\n%s" % (self.get('media', {}).get(
            'title', ""), self.get('synopsis'))

        self._videoId = self.get('videoId')
        self._filename = "%s-Tf1-%s" % (
            datetime.fromtimestamp(self._airDate).strftime("%Y%m%d"),
            self.normalizeProgTitle(self._progTitle))
...
class Tf1Parser(NetworkParser):
    ...
    def parseCollection(self, baseUrl, nbrPage=1, nbrVideoLink=1):
        ...
        while (nbrPage == -1 or index < nbrPage):
            ...
            cardVideos = parsed.find_all(
                "div", attrs={"class": "VideoCard_card__box_k3hxc"})
    ...
    def _findProgPageMetadata(self, page):
        try:
            ...
            for script in scripts:
                ...
                if len(script.contents) > 0 and script.contents[0].startswith(
    ...
    def _getVideoId(self, page):
      ...
      """
        try:
            ...
            pattern = '"__ref":"Video:'
            sindex = script_text.find(pattern)
            sindex += len(pattern)
