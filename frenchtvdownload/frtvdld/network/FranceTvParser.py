...
class FranceTvParser(NetworkParser):
  ...
  def _getVideoId(self, page, getAllUrl=False):
    ...
    try:
      ...
      # using .__str__() hugly hack as .text and .getText() return empty sting for <script> node. 
      vplayer_scripts = [s.__str__() for s in script_tags if (len(s.__str__())>0 and s.__str__().find('window.FTVPlayerVideos')>0)] 
