...
from db.mongoapi import (
    addStream,
    getStreamsByStatus,
    updateStreamWithError,
...
if __name__ == "__main__":
  ...
  parser.add_argument(
      ...
      action="store_true",
      default=False,
      help="save the video metadata in file (.meta)",
  )
  parser.add_argument("--repo",
                      action="store",
                      default="TV",
                      help="define the default folder")
  parser.add_argument(
      "--nocolor",
      action="store_true",
      default=False,
      help="turn of color in terminal",
  )
  parser.add_argument("--version",
                      action="version",
                      version="FrenchTvDownloader %s" % (__version__))
  parser.add_argument(
      "-i",
      "--input",
      action="store",
      choices=["url", "mongo"],
      default="mongo",
      help="input source url or mongo",
  )
  parser.add_argument(
      "urlEmission",
      action="store",
  ...
  for stream in streams:
    ...
    # download if there is more than 10Gb of free space
    if stat.free > (10 * 1073741824):
      downloadOneStream(stream, args)
    else:
      updateStreamWithError(stream, "Not enough free space")
